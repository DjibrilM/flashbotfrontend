...
const ChatItem: React.FC<Props> = ({ id, date, messages }) => {
    ...
    return <RippleButton>
        <div onClick={() => params.id !== id && navigate({ pathname: "conversation/" + id })} className={`w-full flex chatElement border-2 justify-between duration-300 p-3 h-[70px] bg-[#212d3f] rounded-md ${params.id === id ? 'border-[#ffffff24]' : "border-transparent"}`}>
